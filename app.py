...
def load_and_process_data():
    ...
    data = data[['name','price(in Rs.)','ram','storage','display(in inch)','rating']].copy()
    ...
    missing_cols = ['processor','os','no_of_ratings','no_of_reviews']
    for col in missing_cols:
        if col not in data.columns:
            data[col] = 0.0

    # Assign IDs for embeddings
...
# --- Define Hybrid Laptop Recommender ---
...
def recommend_laptops(target_name, top_n=5, price_thresh=0.2, ram_thresh=0.2, storage_thresh=0.2, rating_weight=2.0):
    ...
    for idx in top_candidates:
        ...
        if len(filtered) >= top_n*2:
            break
    if not filtered: filtered = top_candidates
...
def filter_laptops():
    ...
    filtered = filtered[(filtered['raw_ram']>=min_ram) & (filtered['raw_storage']>=min_storage) &
                        (filtered['raw_display']>=min_display) &
                        (filtered['price_usd']<=max_price_usd) &
